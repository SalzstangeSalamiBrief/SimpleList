...
export function parseCSVFromListItemArray(arr: Array<ListItem>) {
  const writeStream = fs.createWriteStream(path.resolve(__dirname, "./csv-output/exportedData.csv"));
  // write col header
  writeStream.write("name, isFavorite, tags \r\n");
  // write rows
  for (let i = 0; i < arr.length; i += 1) {
    let rowString = `${arr[i].name}, ${arr[i].isFavorite
      }, ${arr[i].tags.join(" ")}`;
    if (i < arr.length - 1) {
      rowString += " \r\n";
    }
...
export async function parseListItemArrayFromCSV() {
  ...
  const fileContent: string = await new Promise((resolve, reject) => {
    fs.readFile(path.resolve(__dirname, './csv-output/exportedData.csv'), { encoding: 'utf8' }, (err, data) => {
      if (err) reject(err);
      resolve(data);
    });
  });
